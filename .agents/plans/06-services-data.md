...
## 2. Datasource configuration — ENV vars only (prod), Dev Services for dev/test

`application.properties` references environment variables **only in production**;
dev/test profiles use Quarkus Dev Services to auto-provision Postgres (no manual
DB needed for `quarkusDev` or `./gradlew test`):

```properties
# --- PostgreSQL datasource (values supplied via environment for prod only) ---
quarkus.datasource.db-kind=postgresql
%prod.quarkus.datasource.username=${FT_DB_USER}
%prod.quarkus.datasource.password=${FT_DB_PASSWORD}
%prod.quarkus.datasource.jdbc.url=${FT_DB_URL}
%prod.quarkus.hibernate-orm.schema-management.strategy=none

# Dev Services auto-provision Postgres for dev & test
...
> **Constructor injection caveat:** Kotlin's `noArg` plugin generates a synthetic
...
> **Import SQL column-name requirement:** When applying seed data via
> `sql-load-script` with `schema-management.strategy=drop-and-create`, Hibernate
> generates the table DDL from the entity metadata, which may order columns
> differently from the Docker schema. `INSERT INTO shipment VALUES (...)` without
> column names is **fragile** across different Hibernate versions. Always use
> explicit column names:
> ```sql
> INSERT INTO shipment
>   (pro_number,display_pro,status,origin,destination,shipper,consignee,commodity,
>    weight_lbs,pieces,pickup_time,driver_name,driver_phone,current_location,
>    last_update,estimated_delivery)
> VALUES (...);
> ```
> `tracking_event` INSERTs use `(pro_number,seq,time_label,title,location,state)`
> and are unaffected since they contain no autogenerated columns in the VALUES
> clause.
